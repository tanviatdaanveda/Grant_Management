...
export function NgoSidebar() {
    ...
    <>
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-4 w-4", active ? "text-orange-600 dark:text-orange-400" : "")} />
              {item.label}
            </Link>
          );
        })}
      ...
      <div className="border-t border-gray-200 px-4 py-4 dark:border-gray-700">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-orange-100 text-sm font-semibold text-orange-700 dark:bg-orange-900 dark:text-orange-400">
            {initials}
          </div>
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex lg:w-60 lg:flex-col lg:fixed lg:inset-y-0 border-r border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
        {sidebarContent}
      </aside>
    </>
  );
